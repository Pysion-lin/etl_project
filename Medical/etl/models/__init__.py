...
from Medical.etl.settings.dev import DATABASE_URL,DATABASE_URL_INPUT
# 导入相应的模块
engine = create_engine(DATABASE_URL, max_overflow=5)  # 创建数据库连接，max_overflow指定最大连接数
...
SQLServerDBSession = sessionmaker(bind=engine_SQL_Server)  # 创建DBSession类型
SQLServer_session = SQLServerDBSession()  # 创建session对象
